  ...
  return (
    ...
    <p className="font-semibold  mb-1 hover:scale-105 text-gray-700 uppercase">
      ...
      <p className=" mb-1 hover:scale-105 font-semibold text-gray-700 uppercase">
          Tipo: {""}
          <span className="logoTitle tracking-wider text-2xl font-thin">{pet.tipo}</span>
        </p>        
        <p className="font-semibold mb-1 hover:scale-105 text-gray-700 uppercase">
          Edad: {""}
    ...
    <div className="flex flex-wrap content-center container place-items-center place-content-center  md:justify-between mt-4 mb-1 box-border ">
          <Link to={`/usuario/perfilDelUsuario/${user.id}/editarMascota/${pet.id}`} className="flex  flex-wrap transition-colors logoTitle m-auto text-2xl text-center text-[#221AB7] hover:text-indigo-900">Editar</Link>
          <Link onClick={handleEliminar} className="flex transition-colors flex-wrap m-auto logoTitle   text-red-600 hover:text-red-900 text-2xl text-center">Eliminar
          </Link>
        </div>
