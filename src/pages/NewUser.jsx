...
export async function action({ request }) {
  ...
  const emailDuplicado = usuarios.some(
    (usuario) => usuario.email === datosNuevoUsuario.email
  );
  ...
  if (!usernameRegex.test(username)) {
    errores.push(
      "El campo nombre solo puede contener letras y tiene un minimo de 4 caracteres."
    );
  }
  // Validar contraseña
  if (!passwordRegex.test(password)) {
    errores.push(
      'La contraseña debe tener al menos: 1 mayuscula, 1 minuscula, 1 caracter especial "#$.@", y debe tener de 8 a 16 caracteres'
    );
  }
...
function NewUser() {
  ...
  return (
    <div>
      ...
      <h2 className="text-4xl text-center logoTitle text-[#066aff] my-5">
